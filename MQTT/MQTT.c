...
#define MQTT_BROKER_ADDRESS "10.16.230.161"
//#define MQTT_BROKER_ADDRESS "u-iot-csen-rpi-waterbottle"
#define WICED_TOPIC "light/led1"
#define CLIENT_ID "iotstudent"
        ...
        char *topic, uint8_t qos )
{
    ...
    if ( wait_for_response( WICED_MQTT_EVENT_TYPE_SUBCRIBED,
            MQTT_REQUEST_TIMEOUT ) != WICED_SUCCESS )
    {
        return WICED_ERROR;
    }
...
void application_start( void )
{
    ...
    // Try DNS resolve for up to 5 times.
    for(;DNS_times<5;DNS_times++)
    {
        ...
        if ( ret == WICED_ERROR || broker_address.ip.v4 == 0 )
        {
            if(DNS_times == 4)
            {
                // Quit at the fifth failed try.
                WPRINT_APP_INFO(("Error in resolving DNS. Quit now.\n"));
                return ;
            }
            WPRINT_APP_INFO(("Error in resolving DNS,trying again.\n"));
        }
        else
        {
            break;
